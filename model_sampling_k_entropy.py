...
class SVDHead(nn.Module):
    ...
    def forward(self, *input):
        ...
        # (bs, 1, k)  weights 越小的 熵值越小，对应点越准确
        weights = - torch.sum(torch.mul(scores.log(), scores), dim=-1, keepdim=True).transpose(2, 1).contiguous()
        # 熵值小的权重大
        weights = 1 / (weights + 1e-8)
        # (bs, 3, k)
        src_corr_k = torch.matmul(tgt, scores.transpose(2, 1).contiguous())
        # (bs, 1, k)
